package net.abrikoos.blockout.client.gui.widget;
...
public class BlockoutSlider<T> extends SliderWidget {
    List<T> values;
    double min;
    double max;
    Consumer<T> consumer;

    private BlockoutSlider(int x, int y, int width, int height, Text text, double value, List<T> values, Consumer<T> consumer, double min, double max) {
    ...
    public static <T> BlockoutSlider.Builder builder(Text text, Consumer<T> consumer) {
        return new BlockoutSlider.Builder<>(text, consumer);
    }
...
//                e.printStackTrace();
            ...
            return new BlockoutSlider<T>(x, y, width, height, text, val, values, consumer, min, max);
        }
