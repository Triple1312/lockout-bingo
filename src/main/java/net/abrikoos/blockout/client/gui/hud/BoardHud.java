package net.abrikoos.blockout.client.gui.hud;

import net.abrikoos.blockout.BlockoutLogger;
import net.abrikoos.blockout.client.Board;
import net.abrikoos.blockout.client.ClientGameStateV2;
import net.abrikoos.blockout.client.gui.BlockoutUtils;
...
//import net.abrikoos.blockout.team.UnitedTeamRegistry;
...
//                    BlockoutLogger.log("Error drawing goal " + goals.get(i).goalName() + " at boardhud");
//                }
//            }
            ...
            BlockoutUtils.drawCenteredText(context, client.textRenderer, timeString,topX + (goalwidthheight * 3 + goalpadding * 3 + goalwidthheight/2),bottombarY + goalwidthheight/4, 0xffffffff, false  );
...
//            context.drawTextWithBackground(client.textRenderer, Text.of(String.valueOf(t2)), topX + goalwidthheight + goalpadding + goalwidthheight/2, bottombarY + goalpadding, 200, Colors.get(ClientGameState.getTeams().get(1).teamId));
            ...
            try {
                BlockoutUtils.drawCenteredText(context, client.textRenderer, String.valueOf(ClientGameStateV2.getBoard().scores().get(0)), topX + goalwidthheight/2, bottombarY+ goalwidthheight/4, Colors.get(ClientGameStateV2.getTeams().get(0).teamColor), false);
                BlockoutUtils.drawCenteredText(context, client.textRenderer, String.valueOf(ClientGameStateV2.getBoard().scores().get(1)), topX + 3* goalwidthheight/2 + goalpadding, bottombarY + goalwidthheight/4, Colors.get(ClientGameStateV2.getTeams().get(1).teamColor), false);

            }
            catch (Exception e) {
                BlockoutLogger.log("Error drawing scores in boardhud");
            }
