    ...
    private static final PressAction onPresss = button -> {
        try {
            ...
            if (ClientGameStateV2.teammateRespawnEnabled() && c_connected > 0) {
                ClientGameStateV2.client.setScreen(new net.abrikoos.blockout.client.gui.screens.TeamrespawnScreen());
            } else {
                defaultOnPress.onPress(button);
            }
        }
        catch (Exception ignored) {
            BlockoutLogger.log("pressaction teammate respawn not working idk");
            defaultOnPress.onPress(button);
        }
