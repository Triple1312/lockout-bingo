...
public class Compass2 extends CompassItem {
    ...
    public Text getName(ItemStack stack) {
        String target = stack.get(BlockoutModItems.PLAYER_COMPASS);
        if (target == null) {
...
//                .toList();
        if (playerUUIDs.isEmpty()) {
            stack.set(BlockoutModItems.PLAYER_COMPASS, null);
            return;
        }

        String currentTarget = stack.get(BlockoutModItems.PLAYER_COMPASS);
        int index = -1;
        if (currentTarget != null) {
            index = playerUUIDs.indexOf(currentTarget);
        }
        index = (index + 1) % playerUUIDs.size();
        stack.set(BlockoutModItems.PLAYER_COMPASS, playerUUIDs.get(index));
    }
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        if (world instanceof ServerWorld serverWorld) {
            String tracked__uuid = stack.get(BlockoutModItems.PLAYER_COMPASS);
            if (tracked__uuid == null) {
                ...
                tracked__uuid = stack.get(BlockoutModItems.PLAYER_COMPASS);
            }
            try {
...
//                BlockoutLogger.log("inventory tick compass error");
            }
