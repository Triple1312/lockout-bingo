...
public class AdvancementGoal extends BlockoutGoal {
        ...
        try {
            ...
            if (this.advancementId == null && translatedkey == null) {
                if (validateProgress(null, criterionName)) {
                    completed = player;
                    this.notifyListeners(new BlockoutGoalEvent(player.getUuidAsString(), this.recipiant, this.id)); // how Do I ignore the ally part
                ...
                if (validateProgress(progress, criterionName)) {
                    completed = player;
                    this.notifyListeners(new BlockoutGoalEvent(player.getUuidAsString(), this.recipiant, this.id));
